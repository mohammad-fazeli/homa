import { ipcMain } from "electron";
import { UserCreateInput, UserUpdateInput } from "../db/types";
import { UserModel } from "../db/models/UserModel";
import { CourseModel } from "../db/models/CourseModel";
import { SessionModel } from "../db/models/SessionModel";

export function registerUserHandlers() {
  ipcMain.handle(
    "add-user",
    async (
      event,
      user: UserCreateInput,
      course?: { cost: number; sessions: number },
      sessions?: Date[]
    ) => {
      const newUser = await UserModel.create(user);
      if (course) {
        const newCourse = await CourseModel.create({
          ...course,
          userId: newUser.id,
        });
        sessions?.forEach((session) => {
          SessionModel.create({
            courseId: newCourse.id,
            date: session,
            used: false,
            usedAt: null,
          });
        });
      }
      return newUser;
    }
  );

  ipcMain.handle("get-user", async (event, userId: number) => {
    const user = await UserModel.findById(userId);
    if (!user) throw new Error("کاربر یافت نشد");

    return user;
  });

  ipcMain.handle("get-users", async (event, page: number, limit: number) => {
    return await UserModel.findAll(page, limit);
  });

  ipcMain.handle("update-user", async (event, updatedUser: UserUpdateInput) => {
    const existing = await UserModel.findById(updatedUser.id);
    if (!existing) throw new Error("کاربر یافت نشد");

    return await UserModel.update(updatedUser);
  });

  ipcMain.handle("delete-user", async (event, userId: number) => {
    return await UserModel.delete(userId);
  });
}
