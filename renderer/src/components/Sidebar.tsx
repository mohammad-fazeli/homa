import { NavLink } from "react-router-dom";

const menuItems = [
  { name: "داشبورد", path: "/", icon: "🏠" },
  {
    name: "مدیریت مشتریان",
    path: "/users",
    icon: "👥",
    children: [
      { name: "لیست مشتریان", path: "/users" },
      { name: "افزودن مشتری", path: "/users/new" },
    ],
  },
  { name: "جلسات", path: "/sessions", icon: "📅" },
  { name: "مالی", path: "/billing", icon: "💰" },
  { name: "تنظیمات", path: "/settings", icon: "⚙️" },
];

export default function Sidebar() {
  return (
    <div className="w-64 h-full bg-gray-900 text-white p-4 overflow-y-auto">
      <h2 className="text-xl font-bold mb-6 text-center">هما</h2>
      <ul className="space-y-2">
        {menuItems.map((item) => (
          <li key={item.path}>
            <NavLink
              to={item.path}
              end={item.path === "/" || item.path === "/users"}
              className={({ isActive }) =>
                `flex items-center p-2 rounded-lg cursor-pointer transition ${
                  isActive ? "bg-gray-700" : "hover:bg-gray-800"
                }`
              }
            >
              <span className="text-lg mr-2">{item.icon}</span>
              <span>{item.name}</span>
            </NavLink>
            {item.children && (
              <ul className="ml-6 mt-1 space-y-1 text-sm">
                {item.children.map((sub) => (
                  <li key={sub.path}>
                    <NavLink
                      to={sub.path}
                      end
                      className={({ isActive }) =>
                        `block p-2 rounded-lg transition ${
                          isActive ? "bg-gray-700" : "hover:bg-gray-800"
                        }`
                      }
                    >
                      {sub.name}
                    </NavLink>
                  </li>
                ))}
              </ul>
            )}
          </li>
        ))}
      </ul>
    </div>
  );
}
